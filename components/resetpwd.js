...
import { getsessionactive, resetpwdacton } from '@/app/lib/appwrite';
...
const Resetpwd = () => {
  ...
  const onSubmit = async (data) => {
    await delay(2);
    const result = await resetpwdacton(data.password, window.location.href);
    if (result.success) {
      router.push('/login')
    } else {
      // Handle error
          ...
          <div className='flex flex-col gap-2 max-w-[480px] w-full mx-auto'>
            ...
            >
              <div className='flex flex-col gap-2 text-field-wrapper'>
                ...
                <input
                  {...register('password', { required: true , minLength: {value: 8, message:'Min length is 8'} })}
                  type='password'
